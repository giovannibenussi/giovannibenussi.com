...
description: "It's almost the end of the year and the React team decided to give us a sneak peak of their current research as a Christmas gift: Server Components."
pubDate: "Dec 21 2020"
heroImage: "../../assets/blog/introduction-to-react-server-components/featured-image.png"
---

> **Warning:** Server Components are a research in progress and are by far production-ready. You don't need them to use React nor you shouldn't hurry to learn this. Continue reading just if you're curious about what's going on in the React community.

It's almost the end of the year and the React team decided to give us a sneak peak of their current research as a Christmas gift: Server Components.
...
> React Components are a research in progress that allows you to execute some components on the server and thus, reduce client-size bundle size.
...
![Client components diagram](../../assets/blog/introduction-to-react-server-components/client-components.png)

A blue node represents a node that runs on the client. Let's add some Server Components (represented as orange nodes):

![Client and server components diagram](../../assets/blog/introduction-to-react-server-components/client-and-server-components.png)

As you can see, Server Components gets rendered mixed with client components. **They're not rendered to HTML** but instead they're a representation of a React Component generated on the server so we can render Server Components that contain Client Components.

The main benefit of Server Components is that the bundle size is reduced because you don't need to send code that is used once to the client, but instead just run it on the server.

For example, below is a `Note` component that executes only on the server:

![Bundle size reduction](../../assets/blog/introduction-to-react-server-components/bundle-size-reduction.png)

As you can see, since the `Note` component gets rendered only on the server, we can skip the need to download its dependencies on the browser. The result: zero-bundle-size React Server Components.
...
- Return HTML to the user as fast as possible (at this point the app isn't interactive yet).
...
![Server component response format](../../assets/blog/introduction-to-react-server-components/server-component-response.png)

Here's how it would look formatted (I modify it a bit to have syntax highlight enabled):
...
- **Shared Components**: the components that you currently use. You can execute them both on the client and the server.
...
*Note: Server Components can't have state nor event listeners. If you want interactiveness, you need to use client components.*

Shared components will be executed on the server or the client depending on its parent component:
...
- [Provide modern UX with a server-driven mental model](https://youtu.be/TQQPAU21ZUw?t=2666)
...
React Server Components are not mean to be added to React, but instead they are mean to be added to bundlers like [webpack](https://webpack.js.org/) or [parcel](https://parceljs.org/).
...
[No](https://github.com/reactjs/rfcs/blob/2b3ab544f46f74b9035d7768c143dc2efbacedb6/text/0000-server-components.md#does-this-replace-ssr). You can still have a GraphQL server if it makes sense for your application. In fact, some of Facebook's Server Components use GraphQL.

**Do I need to learn Server Components to use React in the future?**

[No](https://github.com/reactjs/rfcs/blob/2b3ab544f46f74b9035d7768c143dc2efbacedb6/text/0000-server-components.md#do-i-have-to-use-this). Server Components are completely opt-in so current React apps will not be affected by them.
...
Yes. Server Components only execute on the server and never reach the user's browser so it's safe to run database queries on them. Remember that you still need a wrapper like `react-pg` to be able to run queries on Server Components.
